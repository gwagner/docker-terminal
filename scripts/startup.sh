...
CMD+=" --mount type=bind,source=\"/var/run/docker.sock\",target=\"/var/run/docker.sock\""
...
CMD+=" --mount type=bind,source=\"$HOME/terminal-scripts/.motd\",target=\"$DOCKER_HOME_DIR/.motd\""
